...
function Post() {
  ...
  useEffect(() => {
    ...
    if (slug) {
      service.getPost(slug).then((post) => {
        // getPost service will give a post. IF post is present then set its value in the useState Post else navigate the user to the Home Page.
        if (post) {
          setPost(post);
        } else {
          navigate("/");
  ...
  return post ? (
    <div className="w-full bg-white">
      <Container>
        <h2
          className="text-4xl font-serif font-semibold m-4 text-red-500 bg-clip-text"
        ...
        <div className="mt-5">
          <img
            ...
            alt={post.title}
            className="rounded-2xl sm:w-1/4 w-2/3 m-2 shadow-gray-400 shadow-md"
          />
          {isAuthor && (
