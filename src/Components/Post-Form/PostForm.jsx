...
*/
  ...
  const submit = async (data) => {
    // Data is the thing that a user have given while adding or updating post
    ...
    if (post) {
      ...
      const file = data.featureImage[0]
        ? await service.uploadFile(data.featureImage[0])
        : null;
    ...
    // ******User is Adding New post (post do not exist)******
      ...
      const file = await service.uploadFile(data.featureImage[0]); // Uploading the Image

      if (file) {
        // If FIle exist create a new Post
  ...
  useEffect(() => {
    const subscription = watch((value, { name }) => {
      // Value contains all the info. about post like title, slug, image
      if (name === "title") {
        // name is the property of value object which has triggered the callback. (which has been changed in the PostForm like title or content or image)
        ...
        <div class="w-full rounded-lg sm:col-span-6">
          <Input
            ...
            onInput={(e) => {
              setValue(
                "slug",
                createSlug(e.currentTarget.value, { shouldValidate: true }),
              );
            }}
          />
        </div>
        ...
        <div class="w-full rounded-lg sm:col-span-6">
          ...
          {post && (
            <div className="sm:w-2/4 mb-4">
              <img
                src={service.getFile(post.featureImage)}
                alt={post.title}
